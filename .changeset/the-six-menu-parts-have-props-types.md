...
and exported from the package entry (#438). Every other composed component in
the package already shipped one; these six did not, and not because the export
was forgotten — the components took their props inline and anonymously, so there
was no name to export. A consumer importing `MenuItemProps` got `TS2305`, and the
README's own claim that every component has a matching props type was false for
exactly these six.
...
**Naming and exporting only.** Each type is the annotation the component already
...
same optionality. Not one menu component gained a prop, lost one, or changed
what it does with any of them, and the rendered output is byte-identical.
