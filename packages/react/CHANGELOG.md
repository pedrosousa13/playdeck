...
**Correction, 2026-09-13:** the `(#598)` and `#598` citations in the `687260f`
and `f42f118` entries below name the wrong issue. The work they describe —
the button row's touch-target floor moving onto a token, and the phone
row-two fix that reads it — is #622, "Fit the control bar in one row on
phones, and rest the bench on the floating theme at every width". #598 is
"A lone VolumeSlider is invisible and unreachable on a fine pointer", an
unrelated issue landed later in #710. Left uncorrected below because this
entry already shipped in the published 1.1.0 package; see #712.
