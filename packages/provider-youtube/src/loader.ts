...
export const loadYouTubeIframeApi = (): Promise<YouTubeIframeApi> => {
  ...
  const load = new Promise<YouTubeIframeApi>((resolve, reject) => {
    ...
    if (!script) {
      ...
      script.src = scriptSrc;
      script.async = true;
      // Set before the append below: the request leaves the moment the
      // element enters the document, so a policy written afterward would be
      // too late to narrow it. Without this, the script follows the
      // embedding page's own policy, the same exposure #334 closed for the
      // Vimeo oEmbed fetch (#775).
