...
Reely does not refuse a media URL because of the address behind it. Loopback,
...
**Reely never fetches a consumer-supplied URL.** The library makes exactly one
...
is issued by the browser from markup Reely renders — `<source src>`,
...
subresources to the private ranges outright, before Reely could have an opinion.
...
policy Reely cannot actually enforce, and a consumer who believes Reely screens
...
Reely's source parsing.

Observability is handled separately and is not part of this rejection:
[#223](https://github.com/pedrosousa13/reely/issues/223) gives the consumer a
...
- **Reely growing a non-browser fetch path.** If any code path ever requests a
...
- **A supported headless or server-side rendering mode**, where Reely knowingly
...
Each of those is a change in what Reely does, not a re-argument of the same
request — raise it as that.

## Prior requests

- [#246](https://github.com/pedrosousa13/reely/issues/246) — Private, loopback
  and link-local media URLs are permitted, so a source can reach the cloud
  metadata service.

Related but distinct:
[#219](https://github.com/pedrosousa13/reely/issues/219) settled the scheme
