...
Playdeck ships no hover preview over the seek bar, and no thumbnail track for one
to draw from. `SeekSlider` reports position and lets you compose whatever you
like on top of it; it does not render a preview, and no provider hands playdeck
...
title. Nothing in playdeck blocks it and nothing new is needed to allow it. Adding
a playdeck-drawn preview would not unlock a capability; it would substitute playdeck's
...
capability — a thumbnail source playdeck could offer — has nothing behind it.
...
that point the question becomes whether playdeck surfaces that track in player
state, which is a different question from whether playdeck draws a preview, and
...
- [#183](https://github.com/pedrosousa13/playdeck/issues/183) — No hover scrub
