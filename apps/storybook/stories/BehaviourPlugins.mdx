...
player state and issues commands, built from nothing but the two hooks every
other consumer already has: `usePlayerState` and `usePlayerActions`. There is
no new library code behind the name — it is a convention, not a component
this package ships.
...
mapper: it watches five kinds of transition and turns each into one discrete
event, delivered through a callback prop rather than to any sink of its own.
