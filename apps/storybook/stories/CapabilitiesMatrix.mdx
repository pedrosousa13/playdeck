...
providers populate that ladder. `setPlaybackRate` has no dedicated primitive,
and is composed by the consumer instead — see the **Reference example**, whose
settings menu builds a playback-rate group over
`Player.SettingsMenu`/`Player.MenuRadioGroup`, gated on its own capability.
`chapters` gates no shipped control either: it says
...
on it rather than on `selectQuality` (#653) -- a menu built over the older,
single capability would render a radio item that silently does nothing when
a viewer chose it.
