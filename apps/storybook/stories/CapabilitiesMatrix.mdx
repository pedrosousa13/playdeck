...
its `rates` prop) instead of a provider-published one — see the
[**Playback rate**](https://playdeck.video/guides/playback-rate/) guide. The
**Reference example**'s settings menu still composes its own playback-rate
group by hand over `Player.SettingsMenu`/`Player.MenuRadioGroup`, for a
consumer who wants it alongside quality under one trigger.
