    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '`shortcuts={false}` turns the layer off entirely — in `global` mode no `document` listener is attached at all. An object overrides individual actions (`shortcuts={{ seekBackward: null, seekForward: null }}` hands the arrows back to a focused native control); every action it does not name keeps its default. WCAG 2.1.4 Character Key Shortcuts requires one or the other of `global` mode, whose keys are live wherever focus is on the page.',
          '',
