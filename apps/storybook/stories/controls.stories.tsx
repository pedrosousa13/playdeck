    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**Accessibility** — groups its child controls, and owns the media keyboard shortcuts while focus is inside it (`global` attaches the same map to the document instead): Space and `k` toggle playback, `ArrowLeft`/`ArrowRight` seek 5s back and forward, `j`/`PageDown` seek 10s back and `l`/`PageUp` 10s forward, `ArrowUp`/`ArrowDown` move the volume by 0.05, and `m`, `f` and `c` toggle muted, fullscreen and captions. While muted the volume arrows act on the muted zero the control is showing rather than on the published volume behind it: `ArrowUp` unmutes and restores that level unchanged (stepping to 0.05 only where the published volume is itself 0), and `ArrowDown` does nothing while still keeping the key. The thumb moves to the restored level at once, while the player is still muted, so the next press compounds on it — muted at 0.5, `ArrowUp` `ArrowUp` lands on 0.55. Both still step a change the player has not answered yet, because that is what the thumb is showing — muted and dragged to 0.3, `ArrowDown` moves to 0.25. The region owns those keys on a focused `<input type="range">` too, so the arrow distances are the same on `SeekSlider` and `VolumeSlider` as off them; text entry still swallows every key, and a focused button keeps Space and `Enter`.',
          '',
