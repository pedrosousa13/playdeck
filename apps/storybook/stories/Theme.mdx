...
you set on an ancestor, because a declaration always wins over inheritance. Reading with fallbacks means you can
...
| Group      | Tokens                                                                                                                                                                                                                                                         |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Color      | `--playdeck-color-surface`, `--playdeck-color-on-surface`, `--playdeck-color-accent`, `--playdeck-color-track`, `--playdeck-color-buffered`, `--playdeck-color-thumb-ring`, `--playdeck-color-focus`, `--playdeck-color-backdrop`, `--playdeck-color-hairline` |
...
| Activation | `--playdeck-activation-size`, `--playdeck-activation-fill`, `--playdeck-activation-border`                                                                                                                                                                     |
...
**The two activation paint tokens are declared, not only read.**
`ActivationButton` writes `background-color` and `border` on itself inline, as
reads of `--playdeck-activation-fill` and `--playdeck-activation-border`, so a
player carrying no stylesheet at all never paints the user agent's own button
face over its poster. Both stylesheets therefore _declare_ that pair on
`[data-playdeck-part='activation']` rather than only reading it — which is what
lets a value you set on the part itself beat the stylesheet's, since the nearest
declaration in the inheritance chain wins. Set them on the part, not on an
ancestor. A `style` prop passed to `ActivationButton` is spread after the
primitive's own styles and wins outright either way.

**`--playdeck-activation-size` is a floor, not a size.** The activation part is
`fit-content` with `--playdeck-activation-size` as its `min-inline-size` and
`min-block-size`, so icon-only content still measures a 4rem badge while a
label grows the box past it.

**`--playdeck-color-hairline` is `docked.css`'s alone.** It paints the 1px rule
between the docked bar and the picture, and the settings and captions menu's own
border. `theme.css` neither reads nor needs it: its scrim supplies both edges.

**`docked.css` reads the same names with defaults of its own.** Its bar is a
neutral surface rather than a scrim over the picture, so the colours it paints
from mostly carry two defaults each — a light one in the cascade's normal
position and a dark one repeated inside a `@media (prefers-color-scheme: dark)`
block at the foot of the file — while a token whose `theme.css` default already
suits both, such as `--playdeck-color-backdrop` behind the media, is read
unchanged. Spacing, control sizing, typography, motion, safe area and the
activation size keep `theme.css`'s defaults throughout. Each file's header
comment lists its own, so read `packages/react/docked.css` for docked's rather
than assuming the theme's.
