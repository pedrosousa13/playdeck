...
A single class of your own outranks any rule in the file. One rule cannot reach
zero: the slider thumb's ring needs a pseudo-element, and Selectors 4 forbids one
inside `:where()`, so that rule carries the pseudo-element's own (0,0,1). Your
single class is (0,1,0), so it still outranks it — and the cascade layer above
