...
Every control accepts children instead of its default. `CaptionsButton` and the
settings trigger default to an icon; the rest default to a text label — `Play`,
`Mute`, `AirPlay`, `Enter fullscreen`, `Enter picture-in-picture`.
...
`svg` it finds inside one via `--reely-control-icon-size`. A default text label
does not fit that square — it wraps and spills out of the row. So pass icon
children to those controls, or size them yourself: raise
