...
- **`data-reely-part`** — the stable name of a part: one named element in the
  rendered DOM (`play-button`, `mute-button`, `volume-slider`, `seek-slider` /
  `seek-slider-input` / `seek-buffered` / `seek-buffered-range`,
  `fullscreen-button`, `pip-button`, `airplay-button`, `time`, `controls`,
  `poster`, `poster-image`, `loading-indicator`, `activation`, `viewport`,
  `media`, `error` / `error-message` / `error-retry`, `captions` /
  `captions-button` / `captions-announcer`). A part name is not a primitive's
  name, and several parts can come from one primitive. `ErrorDisplay` renders
  `error`, and by default `error-message` and a recoverable error's
  `error-retry` inside it. The slashes above only group related names —
  `captions` / `captions-button` / `captions-announcer` spans two primitives.
...
/* Styling a part: a button.
   `data-reely-part` is the hook and `data-state` is the primitive's output, so
...
[data-reely-part='play-button']:hover {
  background: #1d2939;
}

[data-reely-part='play-button']:focus-visible {
  outline: 2px solid #7aa7ff;
  outline-offset: 2px;
}

/* Playing, so the button now offers "Pause". */
[data-reely-part='play-button'][data-state='playing'] {
  background: #2e5aac;
}

/* Autoplay was refused by the browser: draw attention to the one control that
   can start playback, because nothing else will. */
[data-reely-part='play-button'][data-autoplay-state='blocked'] {
...
**A slider** — `Player/SeekSlider`. One part contains three others
(`seek-slider-input`, `seek-buffered`, and a `seek-buffered-range` per range),
and `data-buffering` is a second axis beside `data-state`:
...
[data-reely-part='seek-slider'][data-state='idle'] {
  opacity: 0.5;
}
...
[data-reely-part='seek-slider'][data-buffering='true']
  [data-reely-part='seek-buffered'] {
  background: #6b4a12;
}
```
...
[data-reely-part='poster-image'][data-state='loaded'] {
  opacity: 1;
}
...
[data-reely-part='activation'][data-state='dormant']:hover {
...
[data-reely-part='activation'][data-state='eligible'] {
  opacity: 0.6;
  cursor: default;
}

/* Mid-load, and `aria-disabled`: it must not look pressable. */
[data-reely-part='activation'][data-state='loading-provider'] {
  cursor: progress;
}

/* The child text reads "Retry", and a press does retry — activation acts on
   this state as well as on `dormant`. So colour it as a recovery, and keep the
   affordance. The exception is a configuration error, which is `aria-disabled`
   and says so through that rather than through colour. */
[data-reely-part='activation'][data-state='error'] {
  background: rgb(63 18 18 / 85%);
}

[data-reely-part='activation'][data-state='error']:hover {
...
None of this is `@reely/react/theme.css`, which is a whole opinion you can take
...
provider can measure the media it is playing, Reely writes that media's own
...
sibling of `data-state` rather than of anything in `theme.css`. Reely never
...
/* Reading a value the library writes.
   `--reely-media-aspect-ratio` is an output, not a token: Reely sets it on the
...
| youtube  | **No.** The IFrame API exposes no intrinsic size, and what Reely mounts for it is not a media element, so there is nothing to measure. |

Reely does not guess a ratio it cannot measure, and does not read one out of a
