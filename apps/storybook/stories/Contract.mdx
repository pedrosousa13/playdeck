...
- **`notice` has no default appearance, deliberately.** A Notice is a non-fatal
  ...
  and for a monitoring system to read. It leaves the DOM again when the rejected
  value is replaced with one the allowlist permits: a notice reports a rejection
  that stands, not one that once happened (#330).
