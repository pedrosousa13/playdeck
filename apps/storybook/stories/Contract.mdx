...
- **`notice` has no default appearance, deliberately.** A Notice is a non-fatal
  `configuration` error reporting a value a provider rejected while the
  fall-back it degraded to stands unchanged — nothing stopped working. So
  `ErrorDisplay` renders it without geometry, without stacking and without
  `role="alert"`, and a player that is still playing is never covered by one
  (#319). It carries the error category on `data-state` like `error` does, and
  it is in the DOM for a consumer to place and for a monitoring system to read.
