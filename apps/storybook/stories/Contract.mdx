...
- **`data-reely-part`** — the primitive's stable name (`play-button`,
  `mute-button`, `volume-slider`, `seek-slider` / `seek-slider-input` /
  `seek-buffered` / `seek-buffered-range`,
...
names to look like something. Four parts carry a worked example below — a
button, a slider, an overlay, and one whose `data-state` is the whole point.
Four, not twenty: the lesson is the same for the rest.
