...
hit-tests, and a live-region policy asserting that state transitions announce
and time updates never do. Axe's needs-review bucket is asserted by equality
too, so a new undiagnosed finding fails the run rather than passing quietly.
The [reference example](?path=/docs/reference-player--docs) sets all of that out
in full.
