...
//
// Playwright's CPU throttling is `Emulation.setCPUThrottlingRate` over CDP,
// which reaches chromium only — and WebKit is where #271's seek failure was
// first observed, so the engine with the best evidence of the defect is exactly
// the one a CDP-only technique would drop. This is built out of what every
// engine already has instead: a task that burns a slice of CPU and then
// reschedules itself. Anything the page queues while a turn is running — the
// media element's `volumechange`, the state publish it drives, React's commit —
// waits for that turn to end, which is the window a press has to survive.
//
...
// wait this file exists to avoid, arrived at by accident. Pipelined, all of
// them are queued before the renderer next drains, and they land within ~10ms
// of each other on chromium and firefox. Only the keydowns matter to either
// control, and they are dispatched in call order.
...
test('the volume control keeps End, Home and End pressed inside one round trip', async ({
  ...
  // And the presses really did land inside one round trip, rather than the
  // congestion having quietly failed to congest anything: by the third press
  // the media element had announced nothing, so the 0 above was the volume the
  // user had asked for and not one published state had already caught up with.
  expect((await presses(page))[2]?.answered).toBe(0);
...
test('N volume arrow presses inside one round trip move N steps', async ({
  ...
  expect((await presses(page))[4]?.answered).toBe(0);
...
test('the seek control keeps End, Home and End pressed inside one round trip', async ({
  ...
  expect((await presses(page))[1]?.answered).toBe(0);
