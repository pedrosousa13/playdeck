...
/**
 * #598: a `VolumeSlider` with no adjacent `MuteButton` sat at `opacity: 0;
 ...
 * either stylesheet (`apps/storybook/.storybook/theme.tsx`).
 *
 * Red, run against the unfixed stylesheets: the four "at rest" cases below
 * (both themes, chromium and firefox) failed identically --
 * `expect(opacity).toBe('1')` read `'0'` in every one.
 */
...
for (const theme of themes) {
  test(`a standalone volume slider is visible and pointer-reachable at rest, under the ${theme} theme`, async ({
    ...
    // `e2e/a11y.spec.ts`), so this is the assertion the computed-style reads
    // above cannot make on their own: that the slider is the thing a pointer
    // actually lands on at its own centre, rather than merely reporting
    // styles nothing enforces.
