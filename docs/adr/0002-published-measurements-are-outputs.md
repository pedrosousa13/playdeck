...
`--reely-media-aspect-ratio`, and nothing in the library ever reads it back.
...
So the rule: Reely may publish a measurement as a CSS custom property, written
...
one" — the primitive reads, the consumer sets. `--reely-media-aspect-ratio`
...
values, the ARIA a primitive owns. That page already reads as "what Reely tells
...
sent it to that table. The pull was the prefix — it is a `--reely-*` name, and
`--reely-*` names live in the token table. But the prefix is a namespace, not a
...
names `--reely-media-aspect-ratio` as a `--reely-*` name deliberately absent
from the table, says the direction is the reason, and points at `Contract.mdx`.
A reader who goes looking where the other `--reely-*` names are still finds the
trail.
...
- Unknown has to remove the property, never publish a zero or an empty value.
  The consumer's rule is `aspect-ratio: var(--reely-media-aspect-ratio, 16 / 9)`
...
- The measurement travels as plain data and the React layer does the DOM write.
  `@reely/core`'s public types compile under `tsconfig.no-dom.json` with
  ...
  instance too — core measures and reports figures, `@reely/react` decides which
  part they land on and what the property is called. The split is not a
  preference; the no-DOM build fails otherwise.
- `--reely-media-aspect-ratio` is the first and only instance — #174, landed as
