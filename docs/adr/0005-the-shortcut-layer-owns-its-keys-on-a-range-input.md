...
scrubber (`step={1}`) and `VolumeSlider` (`step` 0.05 by default). A range
...
`Home`/`End`. The volume slider keeps `ArrowUp`/`ArrowDown` at 0.05, through
`setVolume` rather than through `step`, plus native `Home`/`End` for 0 and 1.
Neither control loses an axis; one of them changes which keys reach it.
...
- **`ArrowLeft`/`ArrowRight` on a focused volume slider now seek instead of
  changing the volume.** Natively they stepped it by `step`, the same 0.05 that
  `ArrowUp`/`ArrowDown` still apply, so no volume adjustment is lost — but a
...
- **A consumer relying on the seek slider's one-second arrow step loses it**,
  along with any `step` or `onChange` they set on that input through
  `inputProps`, because the layer prevents the default before the input sees the
  key. `shortcuts={{ seekBackward: null, seekForward: null }}` suppresses those
  two bindings and hands the arrows straight back to the input, in either
  scoping mode. Pointer dragging is untouched: both sliders' `step` still
  governs the mouse.
